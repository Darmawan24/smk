<template>
  <div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <DataTable
        title="Nilai UKK"
        description="Kelola nilai UKK siswa (pastikan Data UKK sudah dibuat)"
        :data="ukk"
        :columns="columns"
        :loading="loading"
        empty-message="Belum ada nilai UKK"
        empty-description="Buat Data UKK terlebih dahulu, lalu tambah nilai per siswa."
        :searchable="true"
        @search="handleSearch"
      >
        <template #actions>
...
async function fetchUkk () {
  ...
  try {
    ...
    const res = await axios.get(`/admin/ukk?${params}`)
    ukk.value = res.data?.data ?? res.data ?? []
  } catch (e) {
    toast.error('Gagal mengambil nilai UKK')
    ukk.value = []
  } finally {
    loading.value = false
  }
...
async function fetchSiswa (kelasId) {
  ...
  try {
    const r = await axios.get('/admin/siswa', { params: { status: 'aktif', per_page: 1000 } })
    const all = r.data?.data ?? r.data ?? []
    const filtered = all.filter(s => s.kelas_id == kelasId)
    siswaOptions.value = filtered.map(s => ({ id: s.id, label: `${s.nama_lengkap} (${s.nis})` }))
  } catch (_) {
    siswaOptions.value = []
  }
}
...
async function doDelete () {
  ...
  deleting.value = true
  try {
    await axios.delete(`/admin/ukk/${selectedUkk.value.id}`)
...
async function submitForm () {
  ...
  try {
    if (isEditing.value) {
      await axios.put(`/admin/ukk/${form.id}`, {
