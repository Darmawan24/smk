      ...
      <div class="md:flex md:items-center md:justify-between mb-6">
        <div class="flex-1 min-w-0">
          ...
          <p class="mt-1 text-sm text-gray-500">
            Cek penilaian STS (Sumatif Tengah Semester) siswa berdasarkan mata pelajaran
          </p>
        </div>
      </div>
...
const detailTitle = computed(() => {
  if (selectedMapel.value) {
    return `Detail STS - ${selectedMapel.value.nama_mapel}`
  }
  return 'Detail STS'
})
...
const detailCPList = computed(() => {
  ...
  return detailData.value.capaian_pembelajaran.sort((a, b) => {
    if (a.kode_cp === 'STS') return 1
    if (b.kode_cp === 'STS') return -1
...
const cpHeaders = computed(() => {
  ...
  mataPelajaran.value.forEach(mapel => {
    if (mapel.progress && mapel.progress.progress_data) {
      mapel.progress.progress_data.forEach(item => {
        // Only include STS and CP (exclude SAS)
        if (item.kode_cp !== 'SAS') {
  ...
  // Sort: CP first (CP-1, CP-2, CP-3, etc.), then STS at the end
  const sorted = Array.from(cpSet).sort((a, b) => {
    if (a === 'STS') return 1  // STS goes to end
    if (b === 'STS') return -1  // STS goes to end
...
const getSortedProgressData = (progressData) => {
  ...
  return [...progressData].sort((a, b) => {
    if (a.kode_cp === 'STS') return 1  // STS goes to end
    if (b.kode_cp === 'STS') return -1  // STS goes to end
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await axios.get('/wali-kelas/cek-penilaian/sts', { params })
...
const openDetailModal = async (mapel) => {
  ...
  try {
    ...
    const response = await axios.get(`/wali-kelas/cek-penilaian/sts/${mapel.id}`, {
      params: {
        semester: selectedSemester.value
      }
    })
...
// Nilai akhir = rata-rata dari (rata-rata CP-1, CP-2, dst) dan nilai STS
const getNilaiAkhir = (siswa) => {
  ...
  const nilaiSTS = siswa.nilai_by_cp['STS']?.nilai
  ...
  if (rataRataCP !== null && nilaiSTS != null && nilaiSTS !== '') {
    return (rataRataCP + parseFloat(nilaiSTS)) / 2
  }
  if (rataRataCP !== null) return rataRataCP
  if (nilaiSTS != null && nilaiSTS !== '') return parseFloat(nilaiSTS)
  return null
}
