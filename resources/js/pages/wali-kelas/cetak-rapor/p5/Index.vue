...
const fetchKelas = async () => {
  try {
    ...
    kelasFilterOptions.value = response.data.map(k => ({
      ...k,
      full_name: k.full_name || (k.jurusan?.nama_jurusan ? `${k.nama_kelas} - ${k.jurusan.nama_jurusan}` : (k.nama_kelas || ''))
    }))
  } catch (error) {
...
const fetchRapor = async (page = 1) => {
  try {
    ...
    const response = await axios.get(`/wali-kelas/cetak-rapor/p5?${params.toString()}`)
...
const downloadRapor = async (siswa) => {
  try {
    const params = new URLSearchParams()
    const response = await axios.get(`/wali-kelas/cetak-rapor/p5/${siswa.id}/download?${params.toString()}`, {
      responseType: 'blob'
    })
